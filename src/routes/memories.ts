...
import { json, problem, removeTone, uuid } from '../lib/util'
...
memoryRoutes.get('/events', async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT e.*,
       ...
       FROM events e
      WHERE (?1 IS NULL OR e.clan_id = ?1)
...
memoryRoutes.get('/contradictions', async (c) => {
  ...
  return c.json({ contradictions: rows.results })
})
  ...
  const [mems, persons, advices, events] = await Promise.all([
    c.env.DB.prepare(
      `SELECT m.id, m.content, m.type, m.event_date, m.perspective, ps.full_name AS subject_name
        ...
        WHERE m.status='APPROVED'
          AND (LOWER(m.content) LIKE ?1 OR m.content_no_tone LIKE ?2)
        LIMIT 25`
    )
      .bind(like, likeNoTone)
      .all(),
    ...
    c.env.DB.prepare(
      `SELECT id, full_name, aliases, birth_place, birth_date, death_date, is_alive, photo_url
         FROM persons LIMIT 500`
    ).all(),
    c.env.DB.prepare(
      `SELECT a.id, a.original_text, a.category, p.full_name AS spoken_by
         FROM advices a LEFT JOIN persons p ON p.id = a.spoken_by_person_id LIMIT 500`
    ).all(),
    c.env.DB.prepare(
      `SELECT id, title, event_date, event_type, location FROM events LIMIT 500`
    ).all()
  ])
...
memoryRoutes.get('/advices', async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT a.*, p.full_name AS spoken_by_name, p.photo_url AS spoken_by_photo,
      ...
      WHERE (?1 IS NULL OR a.clan_id = ?1)
...
memoryRoutes.delete('/advices/:id', requireAuth, async (c) => {
  await c.env.DB.prepare(`DELETE FROM advices WHERE id = ?`).bind(c.req.param('id')).run()
  return c.json({ ok: true })
})
...
memoryRoutes.get('/time-capsules', async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT t.*, pa.full_name AS author_name, pr.full_name AS recipient_name
      ...
      WHERE (?1 IS NULL OR t.clan_id = ?1) ORDER BY t.created_at DESC`
  )
    .bind(clanId)
    .all<any>()
  const now = Date.now()
  const capsules = (rows.results || []).map((t) => {
    const releasable =
      t.status === 'RELEASED' ||
